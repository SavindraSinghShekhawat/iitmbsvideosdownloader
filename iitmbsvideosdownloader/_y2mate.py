...
from iitmbsvideosdownloader._functions import wait_for_element_by_class, download_wait, log, newest, beautify_file_name, normalize
...
def download_file(driver, DOWNLOAD_DIRECTORY, i, video_id, titles, yt_video_titles):
    ...
    log("Download Button from Popup Clicked, Downloading...", 3)

    download_wait(DOWNLOAD_DIRECTORY)

    log("Download Assumed", 2)

    if newest(DOWNLOAD_DIRECTORY) is None:
        log("Download failed, Retrying...", 2)
