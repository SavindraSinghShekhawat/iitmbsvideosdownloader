...
from iitmbsvideosdownloader.functions import wait_for_element_by_class, wait_for_element_by_xPath, log
from iitmbsvideosdownloader.variables import WEEK, SLEEP_TIME
from iitmbsvideosdownloader.y2mate import download_files
...
def get_video_titles_and_links(driver):
    ...
    log(f"Side titles - {side_titles_text}")

    index_title = side_titles_text.index(f"Week {WEEK}")
    log(f"Selected side title - {side_titles_text[index_title]}")
    ...
    for i in range(len(sub_items_elements)):
        ...
        result = re.sub(r'<!(-)+>', '', text)
        if result == "Video":
            log(f"--> Grabbing sub side title ({titles[i]}) details...")
            element.click()
            ...
            log(yt_video_title)
            log(f"Youtube title - {yt_video_title}")
...
def downloadSubjectVideos(driver, URL, DOWNLOAD_DIRECTORY):
    driver.get(URL)
    log(f"Getting Titles and links...")
